...
const Home = () => {
  ...
  return (
    <main className="min-h-screen w-full flex items-start justify-center">
      {!showCategories &&
        (
          <div className="nav-icon fixed z-50 left-2 top-2 p-2 bg-gradient-to-br from-[#97FEED] to-[#0B666A] via-[#35A29F] rounded-full drop-shadow-md hover:bg-[#D25380] transition ease-in duration-70 cursor-pointer">
            <PiListFill
      ...
      {showCategories && (
        <div className="custom-scrollbar fixed w-full md:w-[20%] h-screen overflow-y-auto overflow-x-hidden flex flex-col justify-start
         items-center gap-2 bg-gradient-to-br from-[#071952] to-[#35A29F] via-[#0B666A] z-40 inset-0 ">
          <div className="pt-4 px-8 w-full flex justify-between items-center">
            ...
            <AiFillCloseCircle
              className="text-white hover:text-[#D25380] transition text-3xl ease-in duration-70"
              onClick={(e: EventListener) => setShowCategories(false)}
            />
          </div>
            ...
            {categories?.map(
              (categoryItem: string, index: number) => (
                <button
                  ...
                  onClick={(e) => handleSelectCategory(categoryItem)}
                  key={index}
                >
                  {categoryItem}
                </button>
      ...
      <div className="w-full md:w-[50%] flex justify-center md:absolute md:left-[500px]">
